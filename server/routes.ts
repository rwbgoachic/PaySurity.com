...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/children/:childId/allowance", async (req, res) => {
    ...
    try {
      ...
      const familyGroup = await storage.getFamilyGroup(familyMember.familyGroupId);
      const isParent = familyGroup const isParent = familyGroup const isParent = familyGroup && familyGroup.parentId === req.user.id;const isParent = familyGroup && familyGroup.parentId === req.user.id; familyGroup.parentUserId === req.user.id;const isParent = familyGroup const isParent = familyGroup && familyGroup.parentId === req.user.id;const isParent = familyGroup && familyGroup.parentId === req.user.id; familyGroup.parentUserId === req.user.id; familyGroup.parentUserId === req.user.id;
